    ...
    for s in 1..=cp.log_total {
        ...
        for gi in 0..cp.n_eval / m {
            ...
            for j in 0..half {
                ...
                let (ui, vi) = (prev + gs + j, prev + gs + j + half);
                let (oa, ob) = (curr + gs + j, curr + gs + j + half);
                p.push_str(&format!("    u_{s}_{gi}_{j} = data[{ui}]\n"));
                p.push_str(&format!("    t_{s}_{gi}_{j} = {tw} * data[{vi}]\n"));
                p.push_str(&format!("    data[{oa}] = u_{s}_{gi}_{j} + t_{s}_{gi}_{j}\n"));
                p.push_str(&format!("    data[{ob}] = u_{s}_{gi}_{j} - t_{s}_{gi}_{j}\n"));
            }
        }
    }
