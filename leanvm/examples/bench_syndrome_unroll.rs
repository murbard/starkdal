...
fn generate_program(cp: &CircuitParams) -> String {
    ...
    for i in 0..NUM_SYNDROME_CHECKS {
        ...
        p.push_str(&format!(
            "    poseidon16_compress(tree + {root_offset}, ds_{i}, chal_out_{i})\n"
        ));
    }
    p.push_str("\n");
    ...
    let mut w = cp.g_1mn;
    let mut sign = F::ONE;
    let coset_points: Vec<F> = (0..cp.n_eval)
        .map(|_| {
            let v = x;
            x *= cp.omega;
            v
    ...
    let weights: Vec<F> = (0..cp.n_eval)
        ...
        .collect();
    let signs: Vec<F> = (0..cp.n_eval)
        .map(|j| if j % 2 == 0 { F::ONE } else { F::ZERO - F::ONE })
        .collect();
    ...
    for i in 0..NUM_SYNDROME_CHECKS {
        ...
        p.push_str(&format!(
            "    s_{i}_0 = V[0] * {} * (bn_{i} * {} - 1) / (beta_{i} - {})\n",
            fmt_f(weights[0]),
            fmt_f(F::ONE / (cp.g_n * signs[0])),
